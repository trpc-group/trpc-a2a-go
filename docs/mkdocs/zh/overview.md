...
A2A 协议本身见 [协议](protocol.md)；构建配方与运行时契约见 [服务端](server.md)
与 [客户端](client.md)。
...
- **`server`** 终结 wire 层。它鉴权、提供 agent card 供发现、分发 JSON-RPC 方法
  与 SSE 流，并（可选地）在同一端口、同一鉴权链内挂载 legacy v0.2.x 端点。
- **`TaskManager`** 拥有一切有状态的东西：懒创建、轮次 close 规则、取消、会话历
  史、留存、订阅者扇出。`taskmanager/memory` 与 `taskmanager/redis` 实现该接口，
  你也可以自带。
...
`SendStreamingMessage` 实时转发事件，两者都出自同一个 `ProcessMessage`。你通过
包一个 `TaskHandle` 来写它——见 [服务端](server.md)。

## 框架为你做了什么

框架的很大一部分，是那些你不必自己做的协议工作：

- **结果派生**——`SendMessage` 从你的 processor 发的事件里返回 `Task` 或
  `Message`（sealed union）；`SendStreamingMessage` 实时转发事件。一份 processor，
  全部响应形状。
- **懒创建与持久化**——任务在首个任务事件时成立，每个事件先持久化再广播。
- **agent card 归一化**——card 同时带 v1.0 字段与其 v0.2.x 弃用镜像，一张 card
  两代客户端都能读。
- **legacy 翻译**——`compat/v0` 把 v0.2.x 斜杠方法 wire 映射到同一个
  `TaskManager`，保留老默认。

确切的运行时契约——轮次生命周期、取消、历史与留存语义——与 agent 作者指南放在
一起，见 [服务端](server.md)。
...
- [协议](protocol.md)——学习 A2A 本身：agent card、四个 wire 对象、任务状态机、
  交互流程。
- [服务端](server.md)——构建 agent：server、processor、运行时契约，以及每项服务端
  能力。
