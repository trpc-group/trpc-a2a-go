...
| `TaskHandler.AddArtifact(taskID, artifact, isFinal, needMoreData)` | `TaskHandle.AddArtifact(artifact, lastChunk)` —— `isFinal` 对应 `lastChunk`；`append` 标志不再暴露：依赖它的分块流应在裸 channel 上发出设置了 `Append` 的 `protocol.TaskArtifactUpdateEvent` |
