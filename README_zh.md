...
| `TaskHandler.AddArtifact(taskID, artifact, isFinal, needMoreData)` | `needMoreData=false` 时调用 `TaskHandle.AddArtifact(artifact, isFinal)`，`needMoreData=true` 时调用 `TaskHandle.AppendArtifact(artifact, isFinal)`；续块必须复用同一个 `ArtifactID` |
