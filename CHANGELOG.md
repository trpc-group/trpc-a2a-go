...
- HTTP+JSON route matching runs on the escaped path, so a percent-encoded colon stays part of an identifier: `GET /tasks/job%3Acancel` addresses the task named `job:cancel` instead of cancelling `job`, and the client escapes colons in task and config IDs. A tenant named `tasks` is reachable too, whether registered statically or through a provider.
- A server-side failure no longer puts its cause in the HTTP+JSON `error.message`; the detail is logged and the client receives the generic heading.
