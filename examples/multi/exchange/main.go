...
// Package main implements the exchange sub-agent in the multi-agent example.
package main
...
func (p *exchangeProcessor) ProcessMessage(
	...
	response := fmt.Sprintf(
		"Exchange request received: %q. This deterministic example does not call a live rate provider.",
		request,
	)
...
func main() {
	...
	taskManager, err := memory.NewTaskManager(&exchangeProcessor{})
	if err != nil {
	...
	description := "Acknowledges a currency request without external services"
	agentCard := protocol.AgentCard{
		...
		Skills: []protocol.AgentSkill{{
			...
			Examples:    []string{"Convert USD to EUR"},
		}},
	}
	...
	go func() {
		log.Printf("exchange agent listening at %s", agentURL)
