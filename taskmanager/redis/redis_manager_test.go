...
func TestPushNotificationCRUD(t *testing.T) {
	...
	got, err := m.OnPushNotificationGet(context.Background(), protocol.TaskIDParams{ID: "task-push"})
	if err != nil {
		t.Fatalf("OnPushNotificationGet failed: %v", err)
	}
	if got.URL != config.URL {
		t.Errorf("get: unexpected config %+v", got)
	}

	list, err := m.OnPushNotificationList(context.Background(),
		protocol.ListTaskPushNotificationConfigsParams{TaskID: "task-push"})
	if err != nil {
		t.Fatalf("OnPushNotificationList failed: %v", err)
	}
	if len(list.Configs) != 1 || list.Configs[0].URL != config.URL {
		t.Errorf("list: unexpected configs %+v", list.Configs)
	}

	if err := m.OnPushNotificationDelete(context.Background(),
		protocol.DeleteTaskPushNotificationConfigParams{TaskID: "task-push"}); err != nil {
	...
	// Operations on unknown tasks fail with task-not-found.
	if _, err := m.OnPushNotificationSet(context.Background(), protocol.TaskPushNotificationConfig{
		...
		t.Errorf("set on missing task: got %v", err)
	}
	if _, err := m.OnPushNotificationGet(context.Background(),
		protocol.TaskIDParams{ID: "task-nope"}); !errors.Is(err, taskmanager.ErrTaskNotFoundSentinel) {
		t.Errorf("get on missing task: got %v", err)
	}
}
