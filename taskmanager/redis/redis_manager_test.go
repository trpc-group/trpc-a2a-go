...
func TestPushNotificationCRUD(t *testing.T) {
	...
	created, err := m.OnPushNotificationSet(context.Background(), config)
	if err != nil {
		t.Fatalf("OnPushNotificationSet failed: %v", err)
	}
	if created.ID != config.TaskID {
		t.Errorf("default config ID = %q, want task ID %q", created.ID, config.TaskID)
	}
	if created.CreatedAt == "" || created.CreatedAt == config.CreatedAt {
		t.Errorf("CreatedAt must be generated by the server, got %q", created.CreatedAt)
	}
	if created.URL != config.URL {
		t.Errorf("unexpected created config: %+v", created)
	}
	...
	// Repeating Set without an ID updates the task's default config and keeps its
	// server-generated creation time.
	updated, err := m.OnPushNotificationSet(context.Background(), protocol.TaskPushNotificationConfig{
		TaskID:    config.TaskID,
		URL:       "https://example.com/updated",
		CreatedAt: "2099-01-01T00:00:00Z",
	})
	if err != nil {
		t.Fatalf("update default config failed: %v", err)
	}
	if updated.ID != config.TaskID || updated.CreatedAt != created.CreatedAt {
		t.Errorf("default update changed identity or CreatedAt: %+v", updated)
	}
	...
	if extra.ID != "extra" || extra.CreatedAt == "" {
	...
	if len(list.Configs) != 2 {
		t.Fatalf("list: got %d configs, want 2: %+v", len(list.Configs), list.Configs)
	}
	for i := 1; i < len(list.Configs); i++ {
		previous, current := list.Configs[i-1], list.Configs[i]
		if previous.CreatedAt > current.CreatedAt ||
			(previous.CreatedAt == current.CreatedAt && previous.ID > current.ID) {
			t.Fatalf("configs are not sorted by CreatedAt then ID: %+v", list.Configs)
		}
	}

	got, err := m.OnPushNotificationGet(context.Background(),
		protocol.GetTaskPushNotificationConfigParams{TaskID: config.TaskID})
	if err != nil {
		t.Fatalf("OnPushNotificationGet failed: %v", err)
	}
	if got.ID != list.Configs[0].ID {
		t.Errorf("get returned config %q, want sorted first %q", got.ID, list.Configs[0].ID)
	}
	...
	list, err = m.OnPushNotificationList(context.Background(),
		protocol.ListTaskPushNotificationConfigsParams{TaskID: config.TaskID})
	if err != nil || len(list.Configs) != 1 || list.Configs[0].ID != config.TaskID {
		t.Fatalf("list after single delete: configs=%+v err=%v", list.Configs, err)
	}

	if err := m.OnPushNotificationDelete(context.Background(),
		protocol.DeleteTaskPushNotificationConfigParams{TaskID: config.TaskID}); err != nil {
		t.Fatalf("OnPushNotificationDelete failed: %v", err)
	}
	if mr.Exists(pushNotificationPrefix + config.TaskID) {
		t.Error("push config key still present after delete")
	}
	list, err = m.OnPushNotificationList(context.Background(),
		protocol.ListTaskPushNotificationConfigsParams{TaskID: config.TaskID})
	if err != nil {
	...
	if len(list.Configs) != 0 {
		t.Errorf("list after delete: got %d configs, want 0", len(list.Configs))
	}
	if _, err := m.OnPushNotificationGet(context.Background(),
		protocol.GetTaskPushNotificationConfigParams{TaskID: config.TaskID}); !errors.Is(err, taskmanager.ErrPushConfigNotFoundSentinel) {
		t.Errorf("get after delete: got %v, want PushConfigNotFound", err)
	}

	// Config registration is independent of lazy task persistence. Get returns a
	// registered config even before a task snapshot exists; after deletion it
	// can distinguish the missing task from a task with no config.
	unknownConfig, err := m.OnPushNotificationSet(context.Background(), protocol.TaskPushNotificationConfig{
		TaskID: "task-nope", URL: "https://example.com",
	})
	if err != nil {
		t.Fatalf("set before task persistence: %v", err)
	}
	if got, err := m.OnPushNotificationGet(context.Background(),
		protocol.GetTaskPushNotificationConfigParams{TaskID: "task-nope"}); err != nil || got.ID != unknownConfig.ID {
		t.Fatalf("get config before task persistence: config=%+v err=%v", got, err)
	}
	if list, err := m.OnPushNotificationList(context.Background(),
		protocol.ListTaskPushNotificationConfigsParams{TaskID: "task-nope"}); err != nil || len(list.Configs) != 1 {
		t.Fatalf("list config before task persistence: list=%+v err=%v", list, err)
	}
	if err := m.OnPushNotificationDelete(context.Background(),
		protocol.DeleteTaskPushNotificationConfigParams{TaskID: "task-nope"}); err != nil {
		t.Fatalf("delete config before task persistence: %v", err)
	}
	if list, err := m.OnPushNotificationList(context.Background(),
		protocol.ListTaskPushNotificationConfigsParams{TaskID: "task-nope"}); err != nil || list.Configs == nil || len(list.Configs) != 0 {
		t.Fatalf("list after deleting config for missing task: list=%+v err=%v", list, err)
	}
	if _, err := m.OnPushNotificationGet(context.Background(),
		protocol.GetTaskPushNotificationConfigParams{TaskID: "task-nope"}); !errors.Is(err, taskmanager.ErrTaskNotFoundSentinel) {
		t.Errorf("get after deleting config for missing task: got %v", err)
	}
}
